...
class LoadCell:
    def __init__(self, config, sensor):
        ...
        self._grams_per_count = None
        self._update_grams_per_count()
        # Batch-averaged force for status (not per-sample storage)
        self._force_batch_avgs = collections.deque(maxlen=20)
        self._force_batch_mins = collections.deque(maxlen=20)
        self._force_batch_maxs = collections.deque(maxlen=20)
        self.force_status = config.getboolean('force_status', default=False)
    ...
    def _update_grams_per_count(self):
    ...
    def _handle_do_ready(self, eventtime):
        # MCU bulk capture (probe trigger_analog needs THR sampling). Host
        # force_status only adds gram math for Moonraker get_status polls.
    ...
    def _sensor_data_event(self, msg):
        ...
        gpc = self._grams_per_count
        tare = self.tare_counts
        need_clients = bool(self.clients.client_cbs)
        if self.force_status and gpc is not None and tare is not None:
            batch_grams = [(row[1] - tare) * gpc for row in data]
            if batch_grams:
                self._force_batch_avgs.append(sum(batch_grams) / len(batch_grams))
                self._force_batch_mins.append(min(batch_grams))
                self._force_batch_maxs.append(max(batch_grams))
        if not need_clients:
            return True
        samples = []
        for row in data:
            grams = (row[1] - tare) * gpc if gpc is not None else None
            samples.append([row[0], grams, row[1], tare])
        self.clients.send({'data': samples, 'errors': errors,
                           'overflows': overflows})
        return True
    ...
    def counts_to_grams(self, sample):
        ...
        if gpc is None:
            return None
        return (sample - self.tare_counts) * gpc
    ...
    def _force_g(self):
        if not (self.force_status and self._grams_per_count is not None
                and len(self._force_batch_avgs) > 0):
            return {}
        return {"force_g": round(avg(self._force_batch_avgs), 1),
                "min_force_g": round(min(self._force_batch_mins), 1),
                "max_force_g": round(max(self._force_batch_maxs), 1)}
